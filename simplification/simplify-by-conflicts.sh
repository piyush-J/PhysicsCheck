...
do
	conf_left=$(echo $m-$conf_used | bc)
	./gen_cubes/concat-edge.sh $o simp/"$f".simp"$i" simp/"$f".ext"$i" | ./cadical/build/cadical /dev/stdin -o simp/"$f".simp$((i+1)) -e simp/"$f".ext$((i+1)) -n -c $conf_left | tee log/"$f".simp$((i+1))
	# ./gen_cubes/concat-edge.sh $o simp/"$f".simp"$i" simp/"$f".ext"$i" | ./drat-trim/drat-trim /dev/stdin -f | tee log/"$f".simp$((i+1)).verify
	# if ! grep -E "s DERIVATION|s VERIFIED" -q log/"$f".simp1.verify
	# then
	# 	echo "ERROR: Proof not verified"
	# fi
